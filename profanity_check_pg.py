...
class DatabaseConnection:
    ...
    def get_connection(self):
        """Create PostgreSQL connection using credentials from Secrets Manager"""
        try:
...
class ProductionModerationService:
    ...
    def fetch_complaints_from_db(self, limit: int = 100, status: str = 'pending') -> List[Dict]:
        """Fetch unprocessed complaints from PostgreSQL"""
        connection = None
        try:
    ...
    def process_complaint_from_db(self, db_record: Dict) -> Optional[Dict]:
        ...
        try:
            # Get JSON data directly from database
            complaint_json = db_record.get('complaint_data')
            
            if not complaint_json:
                logger.warning(f"No complaint_data found for complaint ID: {db_record['id']}")
                return None
            
            # Parse JSON if it's a string
            if isinstance(complaint_json, str):
                try:
                    json_data = json.loads(complaint_json)
                except json.JSONDecodeError as e:
                    logger.error(f"Invalid JSON in complaint_data for ID {db_record['id']}: {e}")
                    return None
            else:
                json_data = complaint_json
    ...
    def _extract_complaint_text(self, json_data: Dict) -> str:
        ...
        try:
            all_text = self._extract_all_text_from_json(json_data)
            if all_text and len(all_text.strip()) >= 5:
                return all_text.strip()
        except Exception as e:
            logger.warning(f"Error extracting all text from JSON: {e}")
        
        return ''

    def _extract_all_text_from_json(self, data: Any, exclude_keys: set = None) -> str:
        """Recursively extract all text content from JSON"""
        if exclude_keys is None:
            exclude_keys = {'id', 'user_id', 'timestamp', 'created_at', 'updated_at', 'status'}
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if key.lower() not in exclude_keys:
                    texts.append(self._extract_all_text_from_json(value, exclude_keys))
        elif isinstance(data, list):
            for item in data:
                texts.append(self._extract_all_text_from_json(item, exclude_keys))
...
"""
    ...
    def update_moderation_status(self, complaint_id: int, status: str, analysis_result: Dict):
        """Update moderation status in PostgreSQL database"""
        connection = None
        try:
            connection = self.db.get_connection()
            ...
            try:
                ...
                if not complaint_data:
                    moderation_service.update_moderation_status(
                        ...
                        'failed_processing', 
                        {
                            'error': 'Failed to extract complaint text from JSON data',
...
# DB_SECRET_NAME: Secrets Manager secret name for PostgreSQL credentials
# AWS_REGION: AWS region
# SNS_TOPIC_ARN: SNS topic for notifications
# FLAGGED_CONTENT_BUCKET: S3 bucket for flagged content
# BEDROCK_MODEL_ID: Bedrock model ID (optional)
# AI_USAGE_THRESHOLD: When to use AI (optional, default 0.6)
# SEVERITY_THRESHOLD: Flagging threshold (optional, default 3)

# Additional IAM Permissions needed:
# - secretsmanager:GetSecretValue
# - bedrock:InvokeModel
# - VPC permissions if database is in VPC
